...
function createCompleter(repl) {
    ...
    const completer = function(line) {
        ...
        // If no hits and line starts with a dot, check for directives
        if (hits.length === 0 && line.startsWith('.')) {
            hits = completionSources.directives.filter(dir => dir.startsWith(line.toLowerCase()));
        }
        
        // If no hits and line starts with 'r', check for registers
        if (hits.length === 0 && line.startsWith('r')) {
